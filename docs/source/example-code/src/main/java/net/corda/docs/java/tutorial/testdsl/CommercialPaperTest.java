...
public class CommercialPaperTest {
    private static final TestIdentity ALICE = new TestIdentity(new CordaX500Name("Alice Corp", "Madrid", "ES"), 70L);
    private static final TestIdentity BIG_CORP = new TestIdentity(new CordaX500Name("BigCorporation", "New York", "US"));
    private static final TestIdentity BOB = new TestIdentity(new CordaX500Name("Bob Plc", "Rome", "IT"), 80L);
    private static final TestIdentity MEGA_CORP = new TestIdentity(new CordaX500Name("MegaCorp", "London", "GB"));
    private static final Party DUMMY_NOTARY = new TestIdentity(getDUMMY_NOTARY_NAME(), (long) 20).getParty();

    private static Party getALICE() {
        return ALICE.getParty();
    }

    private static PublicKey getALICE_PUBKEY() {
        return ALICE.getPubkey();
    }

    private static PublicKey getBIG_CORP_PUBKEY() {
        return BIG_CORP.getPubkey();
    }

    private static Party getBOB() {
        return BOB.getParty();
    }

    @Rule
    public final SerializationEnvironmentRule testSerialization = new SerializationEnvironmentRule();
    private final byte[] defaultRef = new byte[]{123};
